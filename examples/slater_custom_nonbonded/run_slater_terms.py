...
def parse_force_section(root, force_name):
    ...
    mscales = [float(node.attrib.get(f"mScale1{i}", default)) for i, default in zip(range(2, 7), [0.0, 0.0, 0.0, 1.0, 1.0])]
...
def bond_energy_expression(force_name):
    ...
    if force_name == "SlaterDampingForce":
        ...
        return (
            "scale*exp(-(" + x + "))*("
            "(1+(" + x + ")+(" + x + ")^2/2+(" + x + ")^3/6+(" + x + ")^4/24+(" + x + ")^5/120+(" + x + ")^6/720)*c6ij/r^6"
            "+(1+(" + x + ")+(" + x + ")^2/2+(" + x + ")^3/6+(" + x + ")^4/24+(" + x + ")^5/120+(" + x + ")^6/720+(" + x + ")^7/5040+(" + x + ")^8/40320)*c8ij/r^8"
            "+(1+(" + x + ")+(" + x + ")^2/2+(" + x + ")^3/6+(" + x + ")^4/24+(" + x + ")^5/120+(" + x + ")^6/720+(" + x + ")^7/5040+(" + x + ")^8/40320+(" + x + ")^9/362880+(" + x + ")^10/3628800)*c10ij/r^10"
            ")"
        )
    raise ValueError(force_name)
            ...
            for j in range(i + 1, len(atom_types)):
                ...
                if force_name == "QqTtDampingForce":
                    ...
                    energy = slater_disp_damping_kernel(
                        ...
                        bi,
                        bj,
                        jnp.array([math.sqrt(pi["C6"] * 1e6)]),
                        jnp.array([math.sqrt(pj["C6"] * 1e6)]),
                        jnp.array([math.sqrt(pi["C8"] * 1e8)]),
                        jnp.array([math.sqrt(pj["C8"] * 1e8)]),
                        jnp.array([math.sqrt(pi["C10"] * 1e10)]),
                        jnp.array([math.sqrt(pj["C10"] * 1e10)]),
                    )
                else:
                    raise ValueError(force_name)
